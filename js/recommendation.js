...
export const Recommendation = {
    ...
    createAdapter: function(allBeers) {
        return {
            getFeedback: async () => {
                ...
                for (const id in userData) {
                    ...
                    if (weight !== 0) {
                        ...
                        feedback.push({ beer, weight, timestamp });
                    }
                }
                return feedback;
            },
            ...
            getDrunkIds: async () => {
                ...
                const userData = Storage.getAllUserData();
                for (const id in userData) {
                    if (userData[id].count > 0) drunk.add(String(id));
                }
                return drunk;
            }
        };
    ...
    getRecommendedBeers: async function(allBeers, options = {}) {
        ...
        const adapter = this.createAdapter(allBeers);
        ...
        if (options.mode === 'manual' && options.manualPrefs) {
            manualProfile = {
                ...
                breweryAffinity: {},
                flavorAffinity: {},
                idealAbv: options.manualPrefs.idealAbv || null
            };
            if (options.manualPrefs.types) {
                options.manualPrefs.types.forEach(t => manualProfile.typeAffinity[t] = 1.0);
            }
            if (options.manualPrefs.flavors) {
                options.manualPrefs.flavors.forEach(f => manualProfile.flavorAffinity[f] = 1.0);
            }
        }

        const { results, profile } = await engine.recommend(adapter, manualProfile);
    ...
    getBeerMatchScore: async function(beer, allBeers) {
        ...
        return engine.scoreBeer(beer, profile);
    },
    ...
    getUserTasteProfile: async function(allBeers) {
        ...
        const topType = Object.entries(profile.typeAffinity).sort((a, b) => b[1] - a[1])[0];
        const topFlavor = Object.entries(profile.flavorAffinity).sort((a, b) => b[1] - a[1])[0];
        ...
        return {
            ...
            archetype,
            archetypeIcon,
            topTypes: Object.entries(profile.typeAffinity).sort((a, b) => b[1] - a[1]).slice(0, 5),
            topBreweries: Object.entries(profile.breweryAffinity).sort((a, b) => b[1] - a[1]).slice(0, 5),
            topFlavors: Object.entries(profile.flavorAffinity).sort((a, b) => b[1] - a[1]).slice(0, 5)
        };
    }
};
