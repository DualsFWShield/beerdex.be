...
export async function updateWidgetData() {
    ...
    try {
        ...
        // --- 2. Monthly Tasting Count (Simplified/Optimized loop) ---
        // Optimization: We could cache this, but for now we keep it simple but separate.
        ...
        // --- 4. Draw Mini-Graph (REDUCED RESOLUTION to prevent crash) ---
        const drawMiniChart = () => {
            const curve = BAC.getBACCurveData();
            if (!curve || curve.length < 2) return '';
            
            const canvas = document.createElement('canvas');
            // Resolution significantly reduced to avoid TransactionTooLargeException ( IPC 1MB limit)
            const width = 300; 
            const height = 120;
            canvas.width = width;
        ...
        // --- 5. Batch status items into a single JSON to reduce bridge overhead ---
